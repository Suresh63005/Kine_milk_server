...
const ViewSubscribeOrderById = asyncHandler(async (req, res) => {
    ...
    try {
        const order = await SubscribeOrder.findOne({where:{id:orderId,store_id:storeId}});
...
const AssignOrderToRider = asyncHandler(async(req,res)=>{
    ...
    try {
        const order = await SubscribeOrder.findOne({where:{id:order_id,status:"Pending"}})
        if(!order){
        ...
        const updatedOrder = await SubscribeOrder.findOne({where:{id:order_id}})
...
module.exports = {FetchSubscribeOrdersByStatus,ViewSubscribeOrderById,AssignOrderToRider};